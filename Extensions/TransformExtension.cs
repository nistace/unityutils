...
namespace Utils.Extensions {
	public static class TransformExtension {
		public static void ClampPosition(this Transform transform, Vector3 minPosition, Vector3 maxPosition) {
			...
			if (currentPosition.x >= minPosition.x && currentPosition.x <= maxPosition.x && currentPosition.y >= minPosition.y && currentPosition.y <= maxPosition.y &&
				currentPosition.z >= minPosition.z && currentPosition.z <= maxPosition.z) return;
